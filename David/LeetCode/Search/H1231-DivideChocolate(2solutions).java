/*
Difficulty: Hard
Problem Link: https://leetcode.com/problems/divide-chocolate/

Description:

You have one chocolate bar that consists of some chunks. Each chunk has its own sweetness given by
the array sweetness.

You want to share the chocolate with your K friends so you start cutting the chocolate bar into K+1
pieces using K cuts, each piece consists of some consecutive chunks.

Being generous, you will eat the piece with the minimum total sweetness and give the other pieces
to your friends.

Find the maximum total sweetness of the piece you can get by cutting the chocolate bar optimally.



Example 1:

Input: sweetness = [1,2,3,4,5,6,7,8,9], K = 5
Output: 6
Explanation: You can divide the chocolate to [1,2,3], [4,5], [6], [7], [8], [9]
Example 2:

Input: sweetness = [5,6,7,8,9,1,2,3,4], K = 8
Output: 1
Explanation: There is only one way to cut the bar into 9 pieces.
Example 3:

Input: sweetness = [1,2,2,1,2,2,1,2,2], K = 2
Output: 5
Explanation: You can divide the chocolate to [1,2,2], [1,2,2], [1,2,2]


Constraints:

0 <= K < sweetness.length <= 10^4
1 <= sweetness[i] <= 10^5

Solution: Very similar to H410 and the other binary search problems with a restriction as a param.
There are two solutions in this case. You can define valid how you want. You can define valid to be
if you can cut K or more chunks, or you can define valid to be if you can cut K or less chunks.

Runtime: O(nlog(sum of sweetness))

Space Complexity: O(1)

*/

class Solution {
    public int maximizeSweetness(int[] sweetness, int K) {
        int min = Integer.MAX_VALUE;
        int max = 0;

        for(int sweet : sweetness) {
            min = Math.min(min, sweet);
            max += sweet;
        }

        int lo = min;
        int hi = max;

        while(lo < hi) {
            int mid = (lo + hi)/2;
            if(valid(sweetness, K + 1, mid)) {
                if(hi == mid) {
                  return hi;
                }
                hi = mid;
            } else {
                lo = mid + 1;
            }
        }
        return lo;
    }

    public boolean valid(int[] sweetness, int K, int min) {
        int pieces = 1;
        int minSum = 0;
        for(int sweet : sweetness) {
            minSum += sweet;
            if(minSum > min) {
                pieces += 1;
                minSum = 0;
                if(pieces > K) {
                    return false;
                }
            }
        }
        return true;
    }
}

//ALTERNATIVELY, if you can get K or more chunks, then it's valid, and you set lo = mid.

class Solution {
    public int maximizeSweetness(int[] sweetness, int K) {
        int min = Integer.MAX_VALUE;
        int max = 0;

        for(int sweet : sweetness) {
            min = Math.min(min, sweet);
            max += sweet;
        }

        int lo = min;
        int hi = max;

        while(lo <= hi) {
            int mid = (lo + hi + 1)/2;
            if(valid(sweetness, K + 1, mid)) {
                if(lo == mid) {
                    return lo;
                }
                lo = mid;
            } else {
                hi = mid - 1;
            }
        }
        return lo;
    }

    public boolean valid(int[] sweetness, int K, int min) {
        int pieces = 0;
        int minSum = 0;
        for(int sweet : sweetness) {
            minSum += sweet;
            if(minSum >= min) {
                pieces += 1;
                minSum = 0;
            }
        }
        if(pieces < K) {
          return false;
        }
        return true;
    }
}
